  ...
  return new Promise(async (resolving, rejecting) => {
    ...
    for (let tempAttachment of attachments) {
      try {
        ...
        console.log(`Can't parse file ${attachments[count]?.pstFilename}`);
      }
      count++;
      let progress = count / attachments.length * 100;
      console.log(`progress ${progress}%...`);
    ...
    //   reject(err);
    // });
    async function parsing(attachment) {
      return new Promise((resolve, reject) => {
        ...
        if (fs.existsSync(attachment?.pstFilename)) {
          pdfParser.loadPDF(attachment?.pstFilename).catch(err => {
            ...
            reject(`Can't parse file ${attachment?.pstFilename}`);
          });
        }
